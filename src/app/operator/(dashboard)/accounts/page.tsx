...
function generateTemporaryPassword() {
  ...
  return [
    ...
    ...Array.from(bytes.slice(3), (value) => all[value % all.length]),
  ].join("")
}
...
export default function AccountManagementPage() {
  ...
  const filteredAccounts = useMemo(() => {
    if (passwordStatus === "all") return accounts
    return accounts.filter((account) => account.passwordStatus === passwordStatus)
  }, [accounts, passwordStatus])
  ...
  const loadAccounts = async (
    targetPage = page,
    filters?: { keyword: string; role: string; activeStatus: string },
  ...
  const handleResetFilters = () => {
    const resetFilters = { keyword: "", role: "all", activeStatus: "all" }
    setKeyword("")
    setRole("all")
    setActiveStatus("all")
  ...
  const handleResetPassword = async () => {
    ...
    try {
      await accountApi.resetAccountPassword(resetAccount.id, { temporaryPassword })
      setResetCredential({ username: resetAccount.username, temporaryPassword })
      ...
      <Dialog open={Boolean(resetAccount)} onOpenChange={(open) => {
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{resetCredential ? "Mật khẩu tạm chỉ hiển thị một lần." : "Hệ thống sẽ tạo mật khẩu tạm mới và buộc user đổi mật khẩu sau login."}</DialogDescription>
          </DialogHeader>
          {resetCredential ? (
            <div className="space-y-4">
              <div className="rounded-md border bg-muted/30 p-4 text-sm">
