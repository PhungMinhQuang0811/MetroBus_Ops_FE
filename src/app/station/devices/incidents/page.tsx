...
import { deviceApi, incidentApi } from "@/lib/api"
import type { Device, Incident, IncidentDetail } from "@/lib/api"
...
export default function StationDeviceIncidentsPage() {
  ...
  const loadFilterOptions = async () => {
    try {
      const devicesRes = await deviceApi.listDevices({ page: 0, size: 100 })
      setDeviceOptions(devicesRes.result.items)
    } catch (error) {
      console.error("Failed to load filter options", error)
      // Fallback mock devices scoped to station 1
      setDeviceOptions([
        {
          id: 1,
          deviceCode: "METRO-001-ST-001-DV-001",
  ...
  const fetchIncidents = async (
    ...
    try {
      // Automatic scoping on backend, so we do not pass stationId
      ...
      console.warn("Incident search API failed, using station-scoped mock data:", error)
      // Fallback mock logic for testing/integration independence (filtered to station 1)
      let mockFiltered = MOCK_INCIDENTS.filter(item => item.stationId === OPERATOR_STATION_ID)
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Theo dõi và truy vết sự cố phát sinh tại các thiết bị soát vé tại ga.</p>
        </div>
        <div>
      ...
      {/* Filters block */}
      <div className="grid gap-3 rounded-md border bg-card p-3 md:grid-cols-[repeat(3,1fr)_auto_auto] lg:grid-cols-[repeat(6,1fr)_auto_auto]">
              ...
              incidents.map((incident) => (
                <TableRow key={incident.id}>
                  ...
                  <TableCell>
                    ...
                    {incident.stationId === OPERATOR_STATION_ID ? "Bến Thành" : `Ga trạm ${incident.stationId}`}
                  </TableCell>
