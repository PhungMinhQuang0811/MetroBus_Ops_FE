...
- `Gửi Cấp 5`: là shortcut phụ trong bảng, chỉ hiện khi lô ở trạng thái `CREATED`; thao tác gửi thuộc UC20.
...
- `Gửi Cấp 5`: không đặt trong modal tạo lô và không đặt trong kết quả tạo thành công. Sau khi tạo xong, user đi vào `Xem chi tiết lô`; tại đó UC20 mới hiển thị action gửi rõ ràng.
- Trong bảng danh sách, có thể giữ `Gửi Cấp 5` như shortcut cho dòng `CREATED`, nhưng không dùng nó thay cho action chính trong chi tiết lô.
...
```text
┌────────────────────────────────────────────────────────────────────────────────────┐
│ Chi tiết lô OP01-20260604-0001                         [Gửi Cấp 5] [Quay lại]    │
├────────────────────────────────────────────────────────────────────────────────────┤
│ Thông tin lô                                                                       │
...
Trong màn chi tiết lô của UC19, nút `Gửi Cấp 5` ở góc phải header chỉ hiển thị khi `status = CREATED`. Nếu lô đã `SUBMITTED`, `ACCEPTED`, `REJECTED` hoặc `FAILED`, vị trí này không hiển thị nút gửi mà chỉ hiển thị trạng thái hiện tại để user không nhầm đây là thao tác của UC19.
...
**Loại:** System integration + thao tác gửi từ chi tiết lô nếu cho phép  
**Phục vụ:** Theo dõi request/response, trạng thái gửi và kết quả C5.

Tên hiển thị UI tiếp tục dùng `Lô dữ liệu đối soát`. UC20 không có menu riêng; actor vào từ `Đối soát dữ liệu > Lô dữ liệu đối soát`, mở chi tiết lô rồi gửi hoặc xem kết quả gửi.
...
#### Màn hình chi tiết lô khi gửi Cấp 5
...
│ Mã lô: OP01-20260604-0001    Trạng thái: CREATED                                  │
│ Khoảng dữ liệu: 2026-06-04 00:00 - 2026-06-04 23:59                                │
│ Số giao dịch: 1,500          Submitted at: --                                      │
├────────────────────────────────────────────────────────────────────────────────────┤
│ Khu vực gửi Cấp 5                                                                  │
│ Lô đang sẵn sàng gửi. Khi gửi thành công, giao dịch trong lô sẽ được đánh dấu      │
│ đã đồng bộ theo kết quả Cấp 5.                                                     │
│ Trạng thái nút: Enabled vì lô đang CREATED và có 1,500 giao dịch.                  │
├────────────────────────────────────────────────────────────────────────────────────┤
...
│ TX-001         | EVT-001  | ST-BT   | QR-001 | TAP_IN   | OPEN_GATE| PENDING       │
└────────────────────────────────────────────────────────────────────────────────────┘
```

Vị trí nút trên UC20:

- Nút `Gửi Cấp 5` đặt ở góc phải header của màn chi tiết lô, cùng hàng với nút `Quay lại`, để user thấy đây là action chính của lô hiện tại.
- Khu vực `Khu vực gửi Cấp 5` nằm ngay dưới `Thông tin lô` để giải thích điều kiện gửi, số giao dịch sẽ gửi và trạng thái enable/disable của nút.
- Trong danh sách lô, `Gửi Cấp 5` chỉ là shortcut phụ trong cột `Actions` cho dòng `CREATED`; bấm vào shortcut này vẫn mở dialog xác nhận của UC20.
- Không đặt `Gửi Cấp 5` trong tab `Giao dịch trong lô`, vì tab đó là xem danh sách transaction, không phải nơi thao tác gửi.
- Không đặt `Gửi Cấp 5` trong tab `Lịch sử gửi Cấp 5`, vì tab đó chỉ để xem request/response đã phát sinh.

Trạng thái nút `Gửi Cấp 5`:

| Điều kiện | Hiển thị nút |
| --- | --- |
| Lô `CREATED` và `transactionCount > 0` | Hiển thị enabled |
| Lô `CREATED` nhưng đang submit API | Hiển thị disabled/loading |
| Lô `SUBMITTED`, `ACCEPTED`, `REJECTED` | Không hiển thị nút gửi |
| Lô `FAILED` | Hiển thị `Gửi lại Cấp 5` nếu nghiệp vụ cho phép retry |
| User không có `BATCH_WRITE` | Không hiển thị nút gửi, chỉ xem |
...
```text
┌──────────────────────────────────────────────────────────────┐
│ Xác nhận gửi lô dữ liệu lên Cấp 5                            │
├──────────────────────────────────────────────────────────────┤
│ Mã lô: OP01-20260604-0001                                    │
│ Số giao dịch: 1,500                                          │
│ Khoảng dữ liệu: 2026-06-04 00:00 - 2026-06-04 23:59          │
│                                                              │
│ Sau khi gửi, hệ thống sẽ lưu request/response và cập nhật    │
│ trạng thái lô theo phản hồi từ Cấp 5.                         │
│                                                              │
│ [Hủy]                                           [Gửi Cấp 5]  │
└──────────────────────────────────────────────────────────────┘
```

Trong lúc gửi, khóa nút `Gửi Cấp 5` và hiển thị trạng thái đang xử lý để tránh submit lặp.

#### Kết quả gửi thành công

```text
┌──────────────────────────────────────────────────────────────────────┐
│ Đã gửi lô dữ liệu lên Cấp 5                                          │
├──────────────────────────────────────────────────────────────────────┤
│ Mã lô: OP01-20260604-0001                                            │
│ Trạng thái: ACCEPTED                                                 │
│ Submitted at: 2026-06-04 23:00                                       │
│ Phản hồi Cấp 5: ACK - Accepted                                       │
│                                                                      │
│ [Về danh sách]                              [Xem lịch sử gửi]        │
└──────────────────────────────────────────────────────────────────────┘
```
...
- Cập nhật transaction trong lô sang `syncStatus = SYNCED` nếu API contract hiện tại giữ rule này.
...
```text
┌──────────────────────────────────────────────────────────────────────┐
│ Gửi lô dữ liệu thất bại                                              │
├──────────────────────────────────────────────────────────────────────┤
│ Mã lô: OP01-20260604-0001                                            │
│ Trạng thái: FAILED                                                   │
│ Lỗi: LEVEL5_UNAVAILABLE                                              │
│ Chi tiết: Không gọi được endpoint Cấp 5/mock                         │
│                                                                      │
│ [Về danh sách] [Xem lịch sử gửi] [Gửi lại nếu được phép]              │
└──────────────────────────────────────────────────────────────────────┘
```

Khi Cấp 5 reject nghiệp vụ:

- Cập nhật lô sang `REJECTED`.
- Lưu response reject vào `integration_exchange_logs`.
- Không tự sửa giao dịch trong lô trên UI; người vận hành xem chi tiết response để xử lý sau.
...
| --- | --- | --- |
| `CREATED` | Lô đã tạo, chưa gửi Cấp 5 | Cho phép `Gửi Cấp 5` |
| `SUBMITTED` | Đã gửi, đang chờ hoặc đã ghi nhận gửi sơ bộ | Chỉ xem |
| `ACCEPTED` | Cấp 5 đã nhận hợp lệ | Chỉ xem |
| `REJECTED` | Cấp 5 từ chối nghiệp vụ | Chỉ xem response |
| `FAILED` | Lỗi kỹ thuật khi gửi | `Gửi lại` chỉ hiện nếu nghiệp vụ cho phép retry |

Retry chỉ hiển thị nếu nghiệp vụ cho phép; gửi trùng phải idempotent theo `batch_code`. Submit lại lô đã `ACCEPTED` không được phép.
